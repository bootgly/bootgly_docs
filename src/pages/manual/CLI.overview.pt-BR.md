...
Nosso framework oferece suporte uma ampla variedade de componentes visuais para experiência do usuário, como os componentes `Alert`, `Fieldset`, `Select`, `Progress`, `Table`, etc.
...
- **`BOOTGLY_TTY`** — componentes interativos se condicionam à constante `BOOTGLY_TTY`, definida no boot como `1`/`0` a partir do ambiente ou, por padrão, de `stream_isatty(STDIN)`. Terminais emulados que alimentam o stdin programaticamente — como o xterm.js dirigindo um runtime WASM — exportam `BOOTGLY_TTY=1` para que componentes como o `Select` permaneçam interativos mesmo com um stdin que não é um TTY real. Capacidades de stream (como executar `stty`) continuam dependendo do stream de fato.

Quando a posição do cursor não pode ser consultada (sem TTY no stdin), os componentes degradam graciosamente: o `Progress` ancora sua renderização com save/restore de cursor ANSI em vez de posicionamento absoluto, e o `Select` renderiza suas opções uma única vez e retorna a seleção padrão quando `BOOTGLY_TTY` resolve para falso (pipes, CI).
