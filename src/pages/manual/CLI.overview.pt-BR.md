...
Quando a posição do cursor não pode ser consultada (sem TTY no stdin), os componentes degradam graciosamente: o `Progress` ancora sua renderização com save/restore de cursor ANSI em vez de posicionamento absoluto, e o `Menu` renderiza seus itens uma única vez e retorna a seleção padrão quando `BOOTGLY_TTY` resolve para falso (pipes, CI).
