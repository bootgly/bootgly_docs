...
The `scan()` method reads a single line from the input stream — bytes are consumed until a line terminator (`\n` or `\r`) or EOF is reached. It returns the line without the terminator, or `false` on immediate EOF. It works on TTYs and pipes, which makes it the line-reading primitive behind interactive components like [Textbox](/manual/CLI/UI/Components/Textbox/overview) and the [Form](/manual/CLI/UX/Components/Form/overview).
