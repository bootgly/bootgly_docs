...
O método `scan()` lê uma única linha do stream de entrada — os bytes são consumidos até um terminador de linha (`\n` ou `\r`) ou EOF. Ele retorna a linha sem o terminador, ou `false` em EOF imediato. Funciona em TTYs e pipes, o que o torna a primitiva de leitura de linha por trás de componentes interativos como o [Question](/manual/CLI/UI/Components/Question/overview) e o [Form](/manual/CLI/UX/Form/overview).
