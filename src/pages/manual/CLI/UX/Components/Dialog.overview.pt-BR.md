...
O componente `Dialog` é uma caixa modal pintada **sobre** a interface em execução: o corpo renderiza através de um [Frame](/manual/CLI/UI/Base/Frame/overview) interno em coordenadas absolutas enquanto uma sessão interativa é dona de cada tecla — nada mais lê o stdin até ela fechar. Ao fechar, a tela é restaurada repintando os componentes [Boxing](/manual/CLI/UI/Base/Frame/overview) que ele cobria — ou o buffer principal inteiro, quando a sessão envolve a tela alternativa. Variantes prontas respondem os casos comuns: `confirm()`, `alert()` e `prompt()`.
...
Uma demo ao vivo está disponível no [showcase](/manual/CLI/UX/Components/Dialog/showcase).
