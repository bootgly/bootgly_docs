...
The demo declares four fields (a validated Text, a masked Secret, a Select and a Confirm). Click the terminal to interact — answer each field, type `↑` then Enter to go back one field, and edit any field from the final summary Select before confirming.

<d-block-terminal engine="bootgly-cli" title="Form — live demo" command="demo 28" height="420">
Fields are asked one at a time inside fieldset frames — cyan legend while editing, dim once answered; the Secret field echoes `•` per character; the summary Fieldset + confirm Select let you edit any field before submitting.
</d-block-terminal>
