...
`add()` creates each tab frame; `switching()` drives the keys — and the pointer: movement hovers the bar labels, a click switches, the wheel cycles — at a fixed clock; inactive tabs drain bounded and reveal their tail when visited.
</d-block-terminal>

On non-interactive output (pipes, CI) the same code renders the active rectangle once.

The component is documented in the [Tabs overview](/manual/CLI/UI/Base/Tabs/overview).
