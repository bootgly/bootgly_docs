...
The Bootgly PHP Framework offers an efficient and flexible autoload system. First, the file that precedes and includes `autoboot.php` must define your working directory based on the same folder where this file is located. The two files that include `autoboot.php` are: `bootgly` and `index.php`. This working directory can be different from the root directory where the Bootgly platform is located as we will see in the following section.
...
You also have the option to only use the basic Bootgly platform and then you will have to use the CLI and WPI interfaces to develop your apps and APIs and in this case, Bootgly's root and working directories will be the same, since the files that include `autoboot.php` will be in the same folder as `autoboot.php`!
