...
The response is sent automatically when `$work()` returns. If the work throws, the route's middlewares that implement `Recovering` are asked to answer — innermost first, then the global pipeline (see *Error Boundaries and Deferred Work* on the [Middlewares](/manual/WPI/HTTP/HTTP_Server_CLI/Middlewares) page); when none does, a `500 Internal Server Error` is returned (`503 Service Unavailable` for a `Response\Timeout`). The boundaries are the chain of the route this request was dispatched with; a deferral started after the pipeline returned (a `Request\Events::Handled` listener, a global middleware after its `$next()`) only reaches the global pipeline, and work that already handed off (an open SSE stream, a nested `defer()`) reaches no boundary at all.

**What the work sees.** `defer()` calls your closure as `function (Response $Response, Request $Request)`: the `Response` is a private clone for this deferral and the `Request` is a *snapshot* of the request taken at the moment of the call — the same object as `$Response->Request`, which is how you reach it from anywhere that only has the `Response` (a nested `defer()`, a resource). The snapshot is handed only to a second parameter that accepts a `Request` (typed `Request`, or untyped and required); a closure that declares an optional second parameter of another type keeps that parameter's default. Method, URI, fields, uploaded files, authentication, identity, claims, attributes and the Session survive every `wait()`. Route parameters live on the Route, not on the request: `defer()` re-binds the matched Route for every segment of the work, so `WPI->Router->Route->Params` keeps answering for this request. Read the request only through the closure parameters — never through a copy of the route's `$Response` kept in a `use ()` or on `$this`: that object is the worker's shared response, and once your work parks its `$Request` is whichever request the worker is serving now, cookies and Session included. The `$Request` your route received is the live per-connection object: the server empties its body and fields as soon as the synchronous cycle ends and reuses it for the next request on that connection, so a `use ($Request)` inside deferred work reads empty fields after the first `wait()` — and its URI, cookies and Session are not emptied: they may already describe a later request. Headers a middleware sets after `$next()` do not apply to a deferred response — the pipeline has returned by then; set them inside the work.

A Session written inside deferred work is persisted when the deferral answers — on success, on error, when it hands off to SSE (right before the wire leaves) and when it hands off to a nested `defer()` (at the handoff itself, whether or not the child ever answers) — and a session first touched after a `wait()` sets its cookie on the deferred response when the work returns normally. The server's own error answer never carries that cookie: `Catcher` builds the `500`/`503` on a fresh `Response`, exactly as it does for a route that throws synchronously, so a session created by a request that fails is persisted under an ID the client is never told — a `Recovering` middleware answering in place on the deferred `Response` keeps it. A cancelled deferral (the client left while it was parked) gets no save point of its own — the server does not persist it. Do not read that as a discard: the Session's own destructor is a safety net, so a write made before the client left can still reach storage later, when the cycle collector reclaims the abandoned generation; when an abandoned request must leave nothing behind, write the Session in the last segment of the work — after the last `wait()` and after any handoff: a write made before a nested `defer()` is persisted at that handoff, so it must go inside the child instead.

`$timeout` bounds, in seconds, how long the work may stay parked (`0` uses the server-wide `deferredTimeout`, where `0` means unbounded): when it elapses a `Response\Timeout` is thrown at the wait point — catch it to answer yourself, let a `Recovering` middleware answer it, or let it answer `503 Service Unavailable`. A parked deferral is never cut by the connection idle reaper. See *Deferred Response Lifecycle* on the [HTTP Server CLI](/manual/WPI/HTTP/HTTP_Server_CLI) page.
