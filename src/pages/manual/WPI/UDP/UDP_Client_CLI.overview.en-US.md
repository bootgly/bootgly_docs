...
The transport lives in `UDP_Client_CLI\Configs`:
...
> `new Configs('127.0.0.1', 9999)` raises a `TypeError` instead of silently binding the wrong values.
...
- `UDP_Client_CLI\Configs` exposes no TLS or DTLS options.
