...
automatically (deduplicated, appended to any existing `Vary`), so CDNs and shared proxies
never reuse one language's body for another client. The internal route-response cache is
partitioned by the negotiated locale as well.
...
directories (normalized). Pure: returns the choice (falling back to `Language::$source`)
