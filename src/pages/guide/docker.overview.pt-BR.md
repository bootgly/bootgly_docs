...
diretório `projects/` montado, então as próximas execuções sem argumentos abrem a CLI
diretamente, e comandos explícitos sempre pulam o wizard.

As flags `-it` são obrigatórias: sem TTY o wizard não abre (o container imprime uma dica
no lugar). Com Compose, defina `stdin_open: true` e `tty: true`.
...
`--server-workers` aceita valores de sweep (`1..24`, `1..24:4`, `1,2,4,8`), cada
um vira um round de execução, e `--results=charts` gera o report em Markdown +
gráficos SVG nativos. Monte um diretório do host para manter os artefatos:
...
`WITH_HYPERF` e `WITH_POSTGRES` (para os loads TechEmpower com banco do oponente `swoole`).
