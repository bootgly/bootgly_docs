...
| `failures` | `array` | **Presente só quando ao menos um caso falhou.** Um objeto por caso falho: `suite` (o nome da suíte — o diretório dela, para uma suíte que nunca carregou ou que recusou um caso focado inexistente), `case` (o índice dele, começando em 1), `file`, `message` (o help da falha da assertion, ou a causa que o runner viu — classe, mensagem e origem de um Throwable, um timeout, um servidor inalcançável) e `elapsed_ms`. Um Throwable que escapa de uma suíte também é um caso falho: o caso que estava rodando — ou o próximo caso sem registro, quando escapou entre dois casos — ou `case: 0` com `file` vazio quando nenhum caso tinha começado ou a suíte nunca carregou. |
...
`--fail-fast` é um switch sem valor (um valor falha com um alerta) e decide o contrato sozinho — `--view` nunca decide, nem o modo agente. Uma suíte que gerencia os próprios casos (os harnesses E2E ao vivo) para onde o runner dela decidir; o run então para ao fim dessa suíte. Sem `--fail-fast`, o harness do servidor HTTP executa todos os casos mesmo quando um falha, estoura o tempo ou tem a requisição lançando exceção — só para quando o servidor não aceita mais requisições, ou quando três timeouts seguidos são seguidos de uma sonda de vida sem resposta (um servidor que segura requisições abertas de propósito ainda responde à sonda, então o run continua), e reporta o resto como `not reached`; o harness do cliente HTTP anda em lock-step com o servidor mock, então para na primeira falha e reporta o resto como `not reached`. As suítes nunca alcançadas são reportadas como skipped, tanto na linha de resumo quanto no documento do agente.
