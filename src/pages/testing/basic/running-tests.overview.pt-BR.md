...
`--fail-fast` é um switch sem valor (um valor falha com um alerta) e decide o contrato sozinho — `--view` nunca decide, nem o modo agente. Uma suíte que gerencia os próprios casos (os harnesses E2E ao vivo) para onde o runner dela decidir; o run então para ao fim dessa suíte. Sem `--fail-fast`, o harness do servidor HTTP executa todos os casos mesmo quando um falha, estoura o tempo ou tem a requisição lançando exceção — só para quando o servidor não aceita mais requisições ou para de responder (três timeouts seguidos), e reporta o resto como `not reached`; o harness do cliente HTTP anda em lock-step com o servidor mock, então para na primeira falha e reporta o resto como `not reached`. As suítes nunca alcançadas são reportadas como skipped, tanto na linha de resumo quanto no documento do agente.
